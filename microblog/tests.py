...
class UserModelCase(unittest.TestCase):
    ...
    def test_password_hashing(self):
        u=User(username='zhengjiani')
        u.set_password('secret')
        self.assertFalse(u.check_password('dog'))
        self.assertTrue(u.check_password('secret'))
    def test_follow(self):
        u1=User(username='john',email='john@examole.com')
        u2=User(username='zhengjiani',email='zhengjiani@example.com')
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
        ...
        self.assertEqual(u1.followed.first().username,'zhengjiani')
